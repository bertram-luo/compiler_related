...
//
//
enum TOK_TYPE {END, L_PAR, R_PAR,  POS, NEG, INT, ID, ADD, SUB, MUL, DIV, EXP, SHL, SHR, LT, LE, GT, GE, EQ, NEQ, AND, XOR, OR, LAN, LOR, ASSIGN};
TOK_TYPE tk = END;
struct Node;
char* start_pos = NULL;
char* next_pos = NULL;
...
void shift_expression(stack<TOK_TYPE>& operators, stack<Node*>& operands);
struct Node{
    TOK_TYPE _type;
    Node* _left;
    Node* _right;
...
TOK_TYPE next(){
    ...
    } else if (*next_pos == '+'){ ++next_pos; tk = ADD;
    } else if (*next_pos == '-'){++next_pos; tk = SUB;
    } else if (*next_pos == '*'){++next_pos; tk = MUL;
    } else if (*next_pos == '/'){++next_pos; tk = DIV;
    } else if (*next_pos == '^'){++next_pos; tk = EXP;
    } else if (*next_pos == '<') {
        if (*(next_pos + 1) == '<'){
            next_pos += 2;
            tk = SHL;
...
Node* Eparser(){
    ...
    stack<Node*> operands;
    operators.push(END);
    shift_expression(operators, operands);
    expect(END);
    return operands.top();
}
void shift_expression(stack<TOK_TYPE>& operators, stack<Node*>& operands){
...
void pushOperator(TOK_TYPE op, stack<TOK_TYPE>& operators, stack<Node*>& operands){
    if (operators.size() == 0) printf("there is fucking no remaining operators\n");
...
bool precedent_over(TOK_TYPE lhs, TOK_TYPE rhs){
    ...
    if (level_lhs > level_rhs){
       ...
       if (lhs == EXP || lhs == ASSIGN) return false;
       else return true;
    } else {
        return false;
    }
